...
def collect_all_terms(nodes: list, lines: list, module_name):
    ...
    for node in nodes:
        if node.info and node.info.term and node.ref.kind == ["ident"] and node.info.term.expected_type is None:
            ...
            if module_name == ["Mathlib.Data.Finsupp.Basic"] and "subtypeDomain_eq_iff" in node.info.term.value:
                print(f"xxxxxx node.info.term.value: {node.info.term.value}, {range}, {node.info.term.type is not None}")
...
def extract_theorems(project, module_name):
    ...
    for _, decl in enumerate(declarations):
        if decl.kind == "theorem":
            ...
            tactic_before = None
            name_array = decl.name                
            term = None
            while term is None and len(name_array) > 0:
                term = find_term(range, ".".join(name_array), terms)
                if len(name_array) == 1 and not name_array[0][0] =="@":
...
def process_module(fd, project, module_name):
    ...
    for theorem in theorems:
        ...
        if theorem_proof is None:
            theorem_proof = theorem.tactic_before
            print(f"xxxxx assign theorem_proof: {theorem_name} {name} tactic_before: {theorem.tactic_before}")
        if theorem_proof is None:
