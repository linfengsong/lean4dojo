...
import pydantic
from pydantic import ConfigDict, Field, ValidationError, model_validator, NonNegativeInt, TypeAdapter
...
class RootModel(BaseModel):
    ...
    #        obj = None
    #        #### put try/except to report problem
    #        try:
    #            obj = json.load(fp)
    #            return cls.from_obj(obj)
    #        except Exception as e:
    #            print(f"XXXX Error loading from_json_file: {filename}: {e}")
    #            print(f"XXXX Error loading json obj: {obj}")
    #            raise 
    ...
    def from_json_file(cls: type[M], filename: AnyPath) -> list[M]:
        try:
            ...
            with open(filename, 'r', encoding='utf-8') as fp:
                obj = fp.read()
                return TypeAdapter(list[cls]).validate_json(obj)
        except ValidationError as e:
            print(f"\n❌ JSON 校驗失敗！檔案: {filename}")
            print(f"❌ 讀取錯誤: {e}")
            # e.json() 會輸出結構化的錯誤樹，indent=2 讓它易於閱讀
            #print(e.json(indent=2)) 
            
            # 或者如果您想要更簡潔的文字路徑描述：
            # for error in e.errors():
            #     print(f"位置: {' -> '.join(str(p) for p in error['loc'])}")
            #     print(f"錯誤: {error['msg']}\n")
            return []
        except Exception as e:
            print(f"\n❌ 讀取檔案時發生一般錯誤: {filename}")
            print(f"❌ 讀取檔案時發生一般錯誤: {e}")
            return []
...
# 1. 修正 StringRange：支援從 JSON 陣列 [start, stop] 讀取
...
class StringRange(NamedTuple):
    ...
    def __get_pydantic_core_schema__(cls, source_type, handler):
        # 修正：使用 no_info_after_validator_function
        return core_schema.no_info_after_validator_function(
            cls._validate,
            core_schema.list_schema(core_schema.int_schema(), min_length=2, max_length=2)
        )

    @classmethod
    def _validate(cls, value):
        # value 是 [int, int] 的 list，解包給 NamedTuple
        return cls(start=value[0], stop=value[1])
...
class Modifiers(BaseModel):
    ...
    is_noncomputable: bool = Field(default=False) ### 4.24 add default
...
class Variable(BaseModel):
    ...
    # 請將這兩個模型的 config 改為寬鬆模式
    model_config = ConfigDict(extra='allow', populate_by_name=True, strict=False)
...
#    kind: LeanName
#    """Syntax kind"""
# 2. 修正 PPSyntaxWithKind：kind 在 JSON 中是陣列
class PPSyntaxWithKind(PPSyntax):
    kind: list[str] # 修正為 list[str] 以匹配 ["ident"]
    original: bool
    range: Optional[StringRange]
...
# 3. 修正 ElabInfo：對應 JSON 中的 info 鍵值
# 3. 更新 ElabInfo
class ElabInfo(BaseModel):
    tactic: Optional[Any] = None
    term: Optional[Any] = None
    completion: Optional[Any] = None
    simple: Optional[str] = None  # 確保接收 'command', 'completion' 等字串標籤
    model_config = ConfigDict(extra='allow', populate_by_name=True, strict=False)
...
# 4. 修正 InfoTree：這是 elab.json 的核心遞迴節點
class InfoTree(BaseModel):
    children: list["InfoTree"] = Field(default_factory=list) # 遞迴指向自己
    info: Optional[ElabInfo] = None
    ref: Optional[PPSyntaxWithKind] = None
    model_config = ConfigDict(alias_generator=to_camel, populate_by_name=True, strict=False)

# 重要：必須執行 rebuild 否則遞迴會失效
InfoTree.model_rebuild()

# 5. 修改原本的 Elaboration 類別以讀取 InfoTree 列表
class Elaboration(RootModel):
    _plugin_name = "elaboration"

    @classmethod
    def from_json_file(cls: type[Self], filename: AnyPath) -> list[InfoTree]:
        """Read a list of InfoTree nodes from an elab.json file"""
        try:
            with open(filename, 'r', encoding='utf-8') as fp:
                obj = json.load(fp)
                # elab.json 本身是一個 list[InfoTree]
                return TypeAdapter(list[InfoTree]).validate_python(obj)
        except Exception as e:
            print(f"XXXX Error parsing elab.json: {e}")
            return []
