...
class LeanProject:
    ...
    def all_lean_paths(self, base_dir) -> list[Path]:
        """Return the packages directory of the project"""
        base_path = Path(base_dir)
        paths = [base_path]
        packages_path = base_path.joinpath(".lake/packages")
    ...
    def path_of_module(self, module_name: LeanName, base_dir: Optional[AnyPath] = None) -> Path:
        ...
        if base_dir is None:
            base_dir = self.root
        paths = self.all_lean_paths(base_dir)
        for path in paths:
            module_path = path / Path(*module_name).with_suffix(".lean")
    ...
    def find_modules(self, base_dir: Optional[AnyPath] = None, include_hidden_dirs: bool = True) -> list[LeanName]:
        ...
        for sub_path in paths:
            for path, dirs, files in os.walk(sub_path):
                relative_path = Path(path).relative_to(sub_path)
    ...
    def filter_finished_modules(self, modules: list[LeanName]) -> list[LeanName]:
        """Filter out modules that have already been processed (i.e. all output files are present)"""
        ret = []
        for m in modules:
            has_all = True
            for plugin in ALL_PLUGINS:
                p = plugin_short_name(plugin)
                output_file = self.output_dir / f"{pp_name(m)}.{p}.json"
                if not output_file.exists():
                    has_all = False
                    break
            if not has_all:
                print(f"module {m} is not finished")
                ret.append(m)
        return ret
    ...
    def batch_run_jixia(
        ...
        *,
        ...
        # modules = self.filter_finished_modules(modules)
    ...
    def has_module_info(self, module: LeanName) -> ModuleInfo:
        filename = f"{pp_name(module)}.mod.json"
        return os.path.isfile(self.output_dir / filename)
    ...
    def has_info(self, module: LeanName, cls: type[M]) -> list[M]:
        filename = f"{pp_name(module)}.{plugin_short_name(cls._plugin_name)}.json"
        return os.path.isfile(self.output_dir  / filename)