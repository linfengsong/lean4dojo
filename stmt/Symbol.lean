...
where
    ...
    | .node info children => Id.run do
      ...
      if let .ofTermInfo ti := info then
        if let some range := ti.stx.getRange? then
          let rawStr := input.extract range.start range.stop
          if !rawStr.isEmpty && isSimpleIdent rawStr && isFullWord input range then
            let thisRef : InfoReference := {
              name := ti.expr.getAppFn.constName?.getD .anonymous
              srcName := rawStr
              range := range
              isCallHead := isHeadConstOfParentApp parentExpr? (ti.expr.getAppFn.constName?.getD .anonymous)
            }
            refs := refs.push thisRef
      return refs
  ...
  filterGarbage (refs : Array InfoReference) : Array InfoReference :=
    ...
    sorted.foldl (init := #[]) fun acc x =>
      if acc.any (fun y => y.range.start == x.range.start) then acc else acc.push x
